...
final class JavaNodes {
    ...
    @Nonnull
    private final JavaDependencies dependencies = new JavaDependencies();
    @Nonnull
    private final JavaAnnotates annotates = new JavaAnnotates(dependencies);
    @Nonnull
    private final JavaTypes types = new JavaTypes(dependencies, annotates);


    @Nonnull
    private final RootNode rootNode = new RootNode();
    @Nonnull
    private final Map<String, Pair<PackageNode, IPackageBinding>> packageNodeMap = new HashMap<>();

    @Nonnull
    private final Map<IBinding, AbstractNode> bindingNodeMap = new HashMap<>();

    @Nonnull
    private final Map<AbstractNode, Pair<
            Pair<InitializerNode, List<InitializerNode.InitializerImpl>>,
            Pair<InitializerNode, List<InitializerNode.InitializerImpl>>>>
            classInitializerMap = new IdentityHashMap<>();

    @Nonnull
    private final List<Pair<ASTNode, AbstractNode>> delayedDependencyWalkers = new LinkedList<>();

    @Nullable
    private SourceFile sourceFile;
    private Map<String, List<XMLNode>> mapXMlDependency;
    private Map<String, String> mapXMlID;
    ...
    void build(@Nonnull SourceFile sourceFile, @Nonnull CompilationUnit compilationUnit, Map<String, List<XMLNode>> mapXMlDependency, Map<String, String> mapXMlID)
            throws JavaCiaException {
        this.sourceFile = sourceFile;
        this.mapXMlDependency = mapXMlDependency;
        this.mapXMlID = mapXMlID;
    ...
    //for xml file in src
    void build(@Nonnull SourceFile sourceFile, @Nonnull org.w3c.dom.Document document) {
        this.sourceFile = sourceFile;
        ...
        XMLNode rootNode = packageNode.createChildXMlNode(sourceFile, document.getDocumentElement().getNodeName(), document.getDocumentElement().getTextContent(), document.getDocumentElement().getChildNodes(),
                document.getDocumentElement().getAttributes(), count);

        traverseLevel(walker, rootNode, false, count);
    ...
    private void traverseLevel(TreeWalker walker, XMLNode parent, boolean isFirstChild, int count) {
        Node currentNode = walker.getCurrentNode();
        if (!isFirstChild) {
            for (Node n = walker.firstChild(); n != null; n = walker.nextSibling()) {
                traverseLevel(walker, parent, true, count);
                count++;
            }
        } else {
            XMLNode xmlNode = parent.createChildXMlNode(sourceFile, currentNode.getNodeName(), currentNode.getTextContent(), currentNode.getChildNodes(), currentNode.getAttributes(), count);
            dependencies.createDependencyToNode(parent, xmlNode, JavaDependency.MEMBER);
            count = 0;
            for (Node n = walker.firstChild(); n != null; n = walker.nextSibling()) {
                traverseLevel(walker, xmlNode, true, count);
                count++;
            }
        }
    ...
    //for mapper file
    void build(@Nonnull SourceFile sourceFile, @Nonnull org.w3c.dom.Document document, String pathFile, Map<String, List<XMLNode>> mapXMlDependency, Map<String, String> mapXMlID) {
        this.sourceFile = sourceFile;
        DocumentTraversal traversal = (DocumentTraversal) document;
        PackageNode packageNode = createPackageNodeFromPath(sourceFile.getRelativePath());
        ...
        XMLNode rootNode = packageNode.createChildXMlNode(sourceFile, document.getDocumentElement().getNodeName(), document.getDocumentElement().getTextContent(), document.getDocumentElement().getChildNodes(),
                document.getDocumentElement().getAttributes(), count);
        System.out.println("unique name: " + rootNode.getUniqueName());

        String namespace = document.getDocumentElement().getAttribute("namespace");
        if (namespace.contains(".")) {
            putInMap(mapXMlDependency, namespace, rootNode, mapXMlID);
        }

        traverseLevel(walker, rootNode, false, mapXMlDependency, namespace, count, mapXMlID);

        dependencies.createDependencyToNode(packageNode, rootNode, JavaDependency.MEMBER);

        if (mapXMlDependency.containsKey(pathFile)) {
            for (int i = 0; i < mapXMlDependency.get(pathFile).size(); i++) {
                dependencies.createDependencyToNode(mapXMlDependency.get(pathFile).get(i), rootNode, JavaDependency.ACCESS);
            }
        } else {
            String keyId = getKeyId(pathFile);
            if (mapXMlID.containsKey(keyId)) {
                for (int i = 0; i < mapXMlDependency.get(mapXMlID.get(keyId)).size(); i++) {
                    dependencies.createDependencyToNode(mapXMlDependency.get(mapXMlID.get(keyId)).get(i), rootNode, JavaDependency.ACCESS);
                }
            }
        }
        this.sourceFile = null;
    ...
    private void traverseLevel(TreeWalker walker, XMLNode parent, boolean isFirstChild, Map<String, List<XMLNode>> mapXMlDependency, String namespace, int count, Map<String, String> mapXMlID) {
        Node currentNode = walker.getCurrentNode();
        if (!isFirstChild) {
            for (Node n = walker.firstChild(); n != null; n = walker.nextSibling()) {
                traverseLevel(walker, parent, true, mapXMlDependency, namespace, count, mapXMlID);
                count++;
            }
        } else {
            XMLNode xmlNode = parent.createChildXMlNode(sourceFile, currentNode.getNodeName(), currentNode.getTextContent(), currentNode.getChildNodes(), currentNode.getAttributes(), count);
            ...
            if (xmlNode.getSimpleName().equals("select")) {
                for (int i = 0; i < listAttributes.getLength(); i++) {
                    if (listAttributes.item(i).getNodeName().equals("resultMap")) {
                        putInMap(mapXMlDependency, listAttributes.item(i).getNodeValue(), xmlNode, mapXMlID);
                    }
                }
            }
                    ...
                    if (listAttributes.item(i).getNodeName().equals("id")) {
                        ...
                        if (mapXMlDependency.containsKey(key)) {
                            for (int j = 0; j < mapXMlDependency.get(key).size(); j++) {
                                dependencies.createDependencyToNode(mapXMlDependency.get(key).get(j), xmlNode, JavaDependency.ACCESS);
                            }
                        } else {
                            String keyId = getKeyId(key);
                            if (mapXMlID.containsKey(keyId)) {
                                for (int j = 0; j < mapXMlDependency.get(mapXMlID.get(keyId)).size(); j++) {
                                    dependencies.createDependencyToNode(mapXMlDependency.get(mapXMlID.get(keyId)).get(j), xmlNode, JavaDependency.ACCESS);
                                }
                            }
                        }
                    }
            ...
            for (int i = 0; i < listAttributes.getLength(); i++) {
                if (listAttributes.item(i).getNodeName().equals("parameterType") || listAttributes.item(i).getNodeName().equals("resultType") || listAttributes.item(i).getNodeName().equals("type")) {
                    ...
                    if (value.contains(".")) {
                        putInMap(mapXMlDependency, value, xmlNode, mapXMlID);
                    }
                    if (mapXMlDependency.containsKey(value) && !value.contains(".")) {
                        for (int j = 0; j < mapXMlDependency.get(value).size(); j++) {
                            dependencies.createDependencyToNode(xmlNode, mapXMlDependency.get(value).get(j), JavaDependency.ACCESS);
                        }
                    } else if (!mapXMlDependency.containsKey(value) && !value.contains(".")) {
                        String keyId = getKeyId(value);
                        if (mapXMlID.containsKey(keyId)) {
                            for (int j = 0; j < mapXMlDependency.get(mapXMlID.get(keyId)).size(); j++) {
                                dependencies.createDependencyToNode(xmlNode, mapXMlDependency.get(mapXMlID.get(keyId)).get(j), JavaDependency.ACCESS);
                            }
                        }
                    }
                    ...
                    if (!namespace.contains(".")) {
                        String value = listAttributes.item(i).getNodeValue();
                        putInMap(mapXMlDependency, namespace + "." + value, xmlNode, mapXMlID);
                    }
                }
            }
            count = 0;
            for (Node n = walker.firstChild(); n != null; n = walker.nextSibling()) {
                traverseLevel(walker, xmlNode, true, mapXMlDependency, namespace, count, mapXMlID);
                count++;
            }
            if (namespace.contains(".") && !xmlNode.getSimpleName().equals("resultMap")) {
                if (listAttributes.getNamedItem("id") != null) {
                    String parameterType = listAttributes.getNamedItem("parameterType") != null ? listAttributes.getNamedItem("parameterType").getNodeValue() : "";
                    if (!parameterType.equals("")) {
                        //get binary name of parameter in configuration
                        if (mapXMlDependency.containsKey(parameterType)) {
                            final List<XMLNode> list = mapXMlDependency.get(parameterType);
                            for (XMLNode node : list) {
                                final NamedNodeMap attributes = node.getAttributes();
                                if (attributes.getNamedItem("type") != null) {
                                    parameterType = attributes.getNamedItem("type").getNodeValue();

                                }
                            }
                        }
                    }
                    String key = listAttributes.getNamedItem("id").getNodeValue() + "(" + parameterType + ")";
                    putInMap(mapXMlDependency, namespace + "." + key, xmlNode, mapXMlID);
                }
            }
        }
        walker.setCurrentNode(currentNode);
    ...
    void build(org.w3c.dom.Document document, @Nonnull String[] sourcePathArray, Map<String, List<XMLNode>> mapXMLDependency, Path sourcePath, Map<String, String> mapXMlID) {
        int count = 0;
        XMLNode rootXMLNode = rootNode.createChildXMlNode(sourceFile, document.getDocumentElement().getNodeName(), document.getDocumentElement().getTextContent(),
                document.getDocumentElement().getChildNodes(), document.getDocumentElement().getAttributes(), count);
        String key = String.valueOf(sourcePath.getFileName());
        putInMap(mapXMLDependency, key, rootXMLNode, mapXMlID);

        DocumentTraversal traversal = (DocumentTraversal) document;
        TreeWalker walker = traversal.createTreeWalker(document.getDocumentElement(),
                NodeFilter.SHOW_ELEMENT, null, true);
        traverseLevel(walker, rootXMLNode, false, sourcePathArray, mapXMLDependency, count, mapXMlID);
        dependencies.createDependencyToNode(rootNode, rootXMLNode, JavaDependency.MEMBER);
    }

    private void traverseLevel(TreeWalker walker, XMLNode parent, boolean isFirstChild, String[] sourcePathArray, Map<String, List<XMLNode>> mapXMLDependency, int count, Map<String, String> mapXMlID) {
        Node currentNode = walker.getCurrentNode();
        if (!isFirstChild) {
            for (Node n = walker.firstChild(); n != null; n = walker.nextSibling()) {
                traverseLevel(walker, parent, true, sourcePathArray, mapXMLDependency, count, mapXMlID);
                count++;
            }
        } else {
            XMLNode xmlNode = parent.createChildXMlNode(sourceFile, currentNode.getNodeName(), currentNode.getTextContent(), currentNode.getChildNodes(), currentNode.getAttributes(), count);
            ...
            if (xmlNode.getSimpleName().equals("typeAlias")) {
                ...
                for (int i = 0; i < listAttributes.getLength(); i++) {
                    if (listAttributes.item(i).getNodeName().equals("resource")) {
                        ...
                        putInMap(mapXMLDependency, path, xmlNode, mapXMlID);
                    }
                }
            }
            count = 0;
            for (Node n = walker.firstChild(); n != null; n = walker.nextSibling()) {
                traverseLevel(walker, xmlNode, true, sourcePathArray, mapXMLDependency, count, mapXMlID);
                count++;
            }
        }
        walker.setCurrentNode(currentNode);
    ...
    public static void putInMap(Map<String, List<XMLNode>> mapXMLDependency, String key, XMLNode xmlNode, Map<String, String> mapXMlID) {
        String keyId = getKeyId(key);
        if (mapXMLDependency.containsKey(key)) {
    ...
    private void parseClassTypeDeclaration(@Nonnull AbstractNode parentNode,
        ...
        if (mapXMlDependency.containsKey(classNode.getBinaryName())) {
            for (int i = 0; i < mapXMlDependency.get(classNode.getBinaryName()).size(); i++) {
                dependencies.createDependencyToNode(mapXMlDependency.get(classNode.getBinaryName()).get(i), classNode, JavaDependency.ACCESS);
            }
        }
    ...
    private void parseInterfaceTypeDeclaration(@Nonnull AbstractNode parentNode, @Nonnull TypeDeclaration typeDeclaration)
        ...
        if (mapXMlDependency.containsKey(interfaceNode.getBinaryName())) {
            for (int i = 0; i < mapXMlDependency.get(interfaceNode.getBinaryName()).size(); i++) {
                dependencies.createDependencyToNode(mapXMlDependency.get(interfaceNode.getBinaryName()).get(i), interfaceNode, JavaDependency.ACCESS);
            }
        }
...
//		sourceFile.add(initializerNode);
    ...
    private void parseMethodDeclaration(@Nonnull AbstractNode parentNode,
        ...
        if (mapXMlDependency.containsKey(methodNode.getUniqueName())) {
            final List<XMLNode> list = mapXMlDependency.get(methodNode.getUniqueName());
            for (XMLNode node : list) {
                dependencies.createDependencyToNode(methodNode, node, JavaDependency.ACCESS);
            }
        } else {
            String keyId = getKeyId(methodNode.getUniqueName());
            if (mapXMlID.containsKey(keyId)) {
                final List<XMLNode> list = mapXMlDependency.get(mapXMlID.get(keyId));
                for (XMLNode node : list) {
                    dependencies.createDependencyToNode(methodNode, node, JavaDependency.ACCESS);
                }
            }
            ...
            private void createDependencyFromInvocation(@Nonnull MethodInvocation node) {
                ...
                if (argumentValue != null && mapXMlDependency.containsKey(argumentValue)) {
                    for (int i = 0; i < mapXMlDependency.get(argumentValue).size(); i++) {
                        dependencies.createDependencyToNode(javaNode, mapXMlDependency.get(argumentValue).get(i), JavaDependency.ACCESS);
                    }
                } else if (argumentValue != null && !mapXMlDependency.containsKey(argumentValue)) {
                    String keyId = getKeyId(argumentValue);
                    if (mapXMlID.containsKey(keyId)) {
                        for (int i = 0; i < mapXMlDependency.get(mapXMlID.get(keyId)).size(); i++) {
                            dependencies.createDependencyToNode(javaNode, mapXMlDependency.get(mapXMlID.get(keyId)).get(i), JavaDependency.ACCESS);
                        }
                    }
                }
            }
