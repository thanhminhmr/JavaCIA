...
public final class SyntheticType extends AbstractType implements JavaSyntheticType {
	...
	private transient @NotNull List<@NotNull AbstractType> bounds = List.of();


	public SyntheticType(@NotNull String description) {
		super(description);
	}
	...
	@Override
	public @NotNull List<@NotNull AbstractType> getBounds() {
		return isFrozen() ? bounds : Collections.unmodifiableList(bounds);
	}

	public void setBounds(@NotNull List<@NotNull AbstractType> bounds) {
		assertNonFrozen();
		this.bounds = bounds;
	}
