...
import io.github.thanhminhmr.javacia.tree.node.JavaClassNode;
import io.github.thanhminhmr.javacia.tree.node.JavaEnumNode;
...
import io.github.thanhminhmr.javacia.tree.type.JavaSyntheticType;
...
enum EntityPartialMatcher {
	...
	NODE(JavaNode.class) {
		@Override
		protected int partialMatchCode(@NotNull JavaIdentifiedEntity entity, boolean identicalMatch) {
		...
		@Override
		protected boolean partialMatch(@NotNull JavaIdentifiedEntity entityA, @NotNull JavaIdentifiedEntity entityB,
				@NotNull EntityMatcher matcher, boolean identicalMatch) {
			...
			for (final Map.Entry<? extends JavaNode, ? extends JavaDependencyCountTable> entry : dependencyToA.entrySet()) {
				...
				final Pair<EntityWrapper, JavaDependencyCountTable> pair = new Pair<>(wrapper, entry.getValue());
...
//	},
	...
	private static final @NotNull EntityPartialMatcher @NotNull [] PARTIAL_MATCHERS = values();


	private final @NotNull Class<? extends JavaIdentifiedEntity> entityClass;


	EntityPartialMatcher(@NotNull Class<? extends JavaIdentifiedEntity> entityClass) {
		this.entityClass = entityClass;
	}


	private static <R> int @NotNull [] createCountWrapper(@Nullable R any) {
		return new int[]{0};
	}


	static int internalMatchCode(@NotNull JavaIdentifiedEntity entity, boolean identicalMatch) {
		int matchCode = 0;
	...
	static boolean internalMatch(@NotNull JavaIdentifiedEntity entityA, @NotNull JavaIdentifiedEntity entityB,
			@NotNull EntityMatcher matcher, boolean identicalMatch) {
		for (final EntityPartialMatcher partialMatcher : PARTIAL_MATCHERS) {
	...
	abstract protected int partialMatchCode(@NotNull JavaIdentifiedEntity entity, boolean identicalMatch);

	abstract protected boolean partialMatch(@NotNull JavaIdentifiedEntity entityA, @NotNull JavaIdentifiedEntity entityB,
			@NotNull EntityMatcher matcher, boolean identicalMatch);
